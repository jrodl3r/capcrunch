...
var PlayersPanel = React.createClass({
  ...
  buildPlayerList: function(players, group) {
    var list = players.map(function(player, i) {
      if (/(active|injured)/.test(player.status)) {
        return (
          <li key={ i + player.id } className="row">
            <div className="item" data-group={group} data-index={i} draggable="true"
              ...
              <div className="inner">
                ...
                <div className="name"
                  onMouseDown={this.blockAction}
                  onMouseEnter={this.blockAction}>{player.lastname}<span>, </span>{player.firstname}</div>
      ...
      } else if (/(inplay|ir|benched)/.test(player.status)) {
    ...
    { this.props.playerGroup.length
      ...
      { this.props.playerType === 'inactive' && this.props.playerData.created.length
        ...
        <div className="loading-list">
          <i className="fa fa-cog fa-spin"></i> Loading</div>
        <div className="list-drag-cover"
          onDragOver={UI.dropEffect}
          onDragEnter={this.props.onListDragEnter}></div>
